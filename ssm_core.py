...
# CRITICAL FIXES FOR YAHOO LIST INDEXING AND TRONGRID CONTRACT ENDPOINTS
...
class SovereignGlobalMonitorV25_6:
    ...
    async def _fetch_yahoo_chart(self, symbol: str, default_key: str):
        """Прямой защищённый шлюз Yahoo Finance v8 (Исправлено)"""
        url = f"https://yahoo.com{symbol}?interval=1d&range=5d"
        try:
            loop = asyncio.get_event_loop()
            req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) SSMCore/25.6"})
            ...
            # ФИКС ИНДЕКСАЦИИ СЛОВАРЯ (result_list - это массив)
    ...
    async def _fetch_trongrid_contract_stream(self):
        """Низкоуровневый Hex ABI парсер логов вызовов контракта USDT на TronGrid (Исправлено)"""
        url = "https://trongrid.io"
        try:
            loop = asyncio.get_event_loop()
            req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (SSM_Monolith/25.6)", "Accept": "application/json"})
            res = await loop.run_in_executor(None, lambda: urllib.request.urlopen(req, timeout=3.5).read())
            data = json.loads(res.decode("utf-8"))
            ...
            for tx in tx_array:
                try:
                    ...
                    # ФИКС ИНДЕКСАЦИИ КОНТРАКТОВ (contracts - это массив)
