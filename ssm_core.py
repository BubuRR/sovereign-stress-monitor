# ======================================================================
# SOVEREIGN STRESS MONITOR (SSM) — GLOBAL MULTI-COUNTRY MATRIX NODE v25.3-FIXED
# ======================================================================
# Fixed: Yahoo Finance URLs, TronGrid URL, list indexing, parsing
# ======================================================================
...
class SovereignGlobalMonitorV25_3:
    RAW_CONFIG = """
    {
      "GLOBAL_DEFAULTS": {
    ...
    def __init__(self):
        ...
        self.csv_file = os.path.join(self.base_dir, "ssm_historical_database.csv")
    ...
    async def _fetch_trongrid_contract_stream(self):
        ...
        try:
            ...
            req = urllib.request.Request(
                url,
                headers={
                    "User-Agent": "Mozilla/5.0 (SSM_Monolith/25.3-FIXED)",
        ...
        try:
            ...
            if live_incoming_tx:
                ...
                if self.onchain_buffer.get_size() == 0:
                    # first run with no data — not a hard failure
            ...
            status_onchain = f"GATEWAYS_DOWN_STALE_CYCLES={self.cache_stale_cycles}"
            use_dynamic = False
            network_log = f"TIMEOUT | {status_onchain}"
