...
class SovereignGlobalMonitorCore:
    def __init__(self, config_path="config/parameters.json"):
        ...
        self.ground = SovereignGroundAutomator(self.config)
        self.cache_stale = 0
        ...
        self._shared_smh = None
        self._shared_dbb = None

    def _yahoo_urls(self, symbol: str):
    ...
    async def _fetch_prices(self, symbol, fallback):
        ...
        urls.extend(self._yahoo_urls(symbol))
    ...
    async def _fetch_usdt(self):
        ...
        try:
            loop = asyncio.get_running_loop()
            raw = await loop.run_in_executor(None, lambda: self._http_get_sync(url, 10.0))
            data = json.loads(raw.decode())
            values = []
    ...
    def _hard_baseline_deficits(self, smh: float, dbb: float) -> dict:
        d = self.defaults
        hb_smh = float(d.get("HARD_BASELINE_SMH", 280.0))
        hb_dbb = float(d.get("HARD_BASELINE_DBB", 16.5))
        cap = float(d.get("DEFICIT_SOFT_CAP", 1.5))
        dbb_ratio = (float(dbb) - hb_dbb) / max(hb_dbb, 1e-9)
        materials_deficit = float(min(max(dbb_ratio / cap, 0.0), 1.0))
        smh_dd = (hb_smh - float(smh)) / max(hb_smh, 1e-9)
        tech_crash = float(min(max(smh_dd / cap, 0.0), 1.0))
        smh_up = (float(smh) - hb_smh) / max(hb_smh, 1e-9)
        tech_excess = float(min(max(smh_up / (cap * 2.0), 0.0), 0.5))
        return {
            "HARD_BASELINE_SMH": hb_smh,
            "HARD_BASELINE_DBB": hb_dbb,
            "MATERIALS_PHYSICAL_DEFICIT": round(materials_deficit, 4),
            "TECH_CRASH_VS_ANCHOR": round(tech_crash, 4),
            "TECH_EXCESS_VS_ANCHOR": round(tech_excess, 4),
        }

    def _land_stress(self, S: float, ground: float) -> float:
        return float(min(max(max(S, ground), 0.0), 1.0))
    ...
    def _combine(self, G: float, S: float, ground: float) -> float:
        ...
        base = (G + S - G * S) if mode == "soft_or" else max(G, S)
        lift = 0.15 * max(ground - base, 0.0)
        return float(min(max(base + lift, 0.0), 1.0))

    def _gap_and_alarms(self, G: float, S: float, ground: float, land: float = None, phys: dict = None) -> dict:
        official = G
        alternative = max(S, ground)
        land = float(alternative if land is None else land)
        gap = float(max(land - official, 0.0))
        ...
        # Secondary: land elevated while market calm → narrative pressure even if gap < thr
        land_watch = float(self.defaults.get("LAND_WATCH", 0.45))
        if land >= land_watch and G < max(land - 0.05, 0.50):
            if "NARRATIVE_LAG" not in alarms:
                alarms.append("NARRATIVE_LAG")
        ...
        if land >= float(self.defaults.get("LAND_CRITICAL", 0.70)):
            alarms.append("LAND_CRITICAL")
        elif land >= float(self.defaults.get("LAND_WATCH", 0.45)):
            alarms.append("LAND_WATCH")
        if phys and float(phys.get("MATERIALS_PHYSICAL_DEFICIT", 0)) >= 0.35:
            alarms.append("PHYSICAL_DEFICIT_VS_HARD_BASELINE")
        return {
            "OFFICIAL_LAYER": round(official * 100, 2),
            "ALTERNATIVE_LAYER": round(alternative * 100, 2),
            "LAND_STRESS": round(land * 100, 2),
            ...
            "WHY": (
                ...
                "Positive gap: official/market calm while land/structural load is higher."
            ),
        }
    ...
    async def execute_monitoring_cycle(self, country_code="US", past_risk=50.0):
        ...
        usdt_med = self.onchain.trimmed_median(float(prior.get("historical_median_usdt", 10000)))
        ...
        if smh_break or dbb_break or hs >= 3:
            if hs == 0:
                self.pre_crisis_smh = float(np.mean(smh_hist[-50:])) if len(smh_hist) >= 50 else smh
                self.pre_crisis_dbb = float(np.mean(dbb_hist[-50:])) if len(dbb_hist) >= 50 else dbb
            self.reference_mode = "pre_crisis"
        else:
            self.reference_mode = "normal"

        act_smh = [self.pre_crisis_smh] * 50 if self.reference_mode == "pre_crisis" else smh_hist
        act_dbb = [self.pre_crisis_dbb] * 50 if self.reference_mode == "pre_crisis" else dbb_hist
        ...
        phys = self._hard_baseline_deficits(smh, dbb)
        w_phys = float(d.get("PHYSICAL_DEFICIT_WEIGHT", 0.35))
        market_linear += w_phys * (
            float(phys["MATERIALS_PHYSICAL_DEFICIT"])
            + 0.5 * float(phys["TECH_CRASH_VS_ANCHOR"])
            + 0.25 * float(phys["TECH_EXCESS_VS_ANCHOR"])
        )
        ...
        land = self._land_stress(S, ground)
        R = self._combine(G, S, ground)
        risk_pct = round(R * 100.0, 2)
        gap_block = self._gap_and_alarms(G, S, ground, land=land, phys=phys)
        ...
        land_crit = float(d.get("LAND_CRITICAL", 0.70))
        land_watch = float(d.get("LAND_WATCH", 0.45))
        if risk_pct >= alert or S >= chronic_crit or ground >= 0.75 or land >= land_crit:
            status = "CRITICAL"
            self.high_stress_dur[code] = hs + 1
        elif risk_pct >= elev or S >= chronic_watch or ground >= 0.45 or land >= land_watch:
            status = "ELEVATED"
        ...
        local_overlay = (
            "STRESS" if S >= chronic_crit or land >= land_crit
            else ("WATCH" if S >= chronic_watch or land >= land_watch else "NONE")
        )
        ...
        return {
            ...
            "OFFICIAL": {
                ...
                "SMH_SOURCE": "LIVE"
                if abs(float(smh) - float(d["SMH_50D_AVERAGE_NORM"])) > 0.5
                else "FALLBACK",
                "DBB_SOURCE": "LIVE"
                if abs(float(dbb) - float(d["DBB_50D_AVERAGE_NORM"])) > 0.5
                else "FALLBACK",
            ...
            "GROUND": {
                ...
                "MODE": ground_pack.get("MODE"),
                "LIVE_CHANNELS": ground_pack.get("LIVE_CHANNELS"),
                "SIGNALS": {
                    k: {
            ...
            "LAND": {
                "LAND_STRESS": round(land * 100, 2),
                "STATUS_DRIVER": (
                    "LAND" if land >= max(G, 0) and land >= float(d.get("LAND_WATCH", 0.45))
                    else "MARKET_OR_MIXED"
                ),
            },
            "HARD_BASELINE": phys,
            ...
            "DRIVERS": {
                ...
                "materials_physical_vs_anchor": phys["MATERIALS_PHYSICAL_DEFICIT"],
                "stablecoin_flow": round(flow, 4),
                "fiscal_prior": round(float(prior.get("fiscal_pressure", 0)), 4),
                "demographic_prior": round(float(prior.get("demographic_squeeze", 0)), 4),
            },
...
async def run_all():
    ...
    for code in countries:
        try:
            ...
            print(
                ...
                f"Land={row.get('LAND', {}).get('LAND_STRESS')} "
                f"Gap={row['COMPARISON']['GAP_SCORE']} alarms=[{alarms}]",
                flush=True,
            )
        except Exception as e:
    ...
    report = {
        ...
        "AI_ADVISORY": None,
        "DISCLAIMER": (
            "Hybrid radar. Ground defaults to priors; ACLED optional. "
            "Not investment advice. Not a full informal-economy census."
        ),
        "METHODOLOGY": {
            "OFFICIAL_G": "SMH/DBB/USDT + change-point anti-drift",
            "STRUCTURAL_S": "S=1-exp(-λ(αF+βD+γB))",
            "GROUND": "SovereignGroundAutomator: 5 signals, prior-first, ACLED if keys",
            "HARD_BASELINE": "HARD_BASELINE_SMH/DBB anchors into G; LAND=max(S,Ground)",
            "AI_ADVISORY": "Optional LLM narrative only; never overrides scores; needs YOUR key",
            "GAP": "max(S, ground) − G",
            "DISPLAY_R": "max(G,S) + small ground lift",
        },
    ...
    # Optional AI advisory (never overrides scores)
    try:
        from core.ai_advisory import SovereignAIAdvisory
        cfg_path = os.path.join(core.root, "config", "parameters.json")
        with open(cfg_path, encoding="utf-8") as _cf:
            _cfg = json.load(_cf)
        _adv = SovereignAIAdvisory(_cfg)
        if _adv.ai.get("enabled"):
            print("[SSM] AI advisory enabled — requesting narrative...", flush=True)
            report["AI_ADVISORY"] = _adv.generate_brief(report)
            report["AI_MODE"] = str(_adv.ai.get("mode") or "compare")
        else:
            report["AI_ADVISORY"] = None
            report["AI_MODE"] = "off"
    except Exception as _ae:
        report["AI_ADVISORY"] = {
            "role": "advisory_not_authoritative",
            "status": f"DEGRADED:{_ae}",
        }
        report["AI_MODE"] = "degraded"
