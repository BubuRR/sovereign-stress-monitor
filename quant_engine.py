...
class SovereignGlobalMonitorCore:
    def __init__(self, config_path="config/parameters.json"):
        ...
        # Per-country crisis duration (shared int across countries was incorrect)
    ...
    async def _fetch_data_stream(self, symbol, fallback_val):
        ...
        if poly_key and "PASTE" not in poly_key:
            ...
            # Correct Yahoo Finance chart API endpoint
            url = (
        ...
        try:
            ...
            req = urllib.request.Request(
                url,
                headers={
                    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) SSMCore/28.7"
                },
            )
    ...
    async def _fetch_tron_usdt_stream(self):
        ...
        if qn_key and "PASTE" not in qn_key:
            ...
            # Public TronGrid endpoint for recent TRC-20 transfers of USDT
            url = (
        ...
        try:
            ...
            # TronGrid v1 events format
            for event in data.get("data", []):
                try:
                    result = event.get("result", {})
                    # value is usually in sun / 1e6 for USDT
                    raw_val = result.get("value") or result.get("_value") or result.get("amount")
                    if raw_val is None:
                        continue
            ...
            # Fallback: try raw_data.contract path (legacy trongrid style)
            if not values:
    ...
    async def execute_monitoring_cycle(self, country_code="US", past_risk=50.0):
        ...
        total_stress = (
            financial + max(kinetic, 0.0) * 1.2 + (past_risk / 100.0) * 0.2
        )
...
async def run_all():
    """
    Master orchestration entry-point used by ssm_core.py and GitHub Actions.
    Iterates the full country matrix, runs one monitoring cycle per country,
    writes ssm_unified_report.json and returns the composite passport.
    """
    ...
    for code in countries:
        try:
            # Seed past_risk from last DB record if available
